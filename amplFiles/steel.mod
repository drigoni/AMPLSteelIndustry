...
param tempiProd{FABBRICHE,MACCHINE,TURNI,PRODOTTI}; #Tempi necessari per produrre una tonnellata di un tipo di acciaio lavorato
...
param costiTrasporto{FABBRICHE,DEPOSITI}; #Costi di trasporto del camion da una fabbrica ad un deposito
...
param richiesteMaxDepositi{DEPOSITI}; #Massima capienza di acciaio lavorato per ogni deposito
...
#-------------- VARIABILI --------------------
#---------------------------------------------
#Rappresenta il numero di tonnellate prodotto lavorato per ogni specifico acciaio, turno, macchina e una fabbrica
var al{FABBRICHE,MACCHINE,TURNI,PRODOTTI} integer >= 0;
#Rappresenta il numero di tonnellate di ogni prodotto lavorato che occorre trasportare da ogni fabbrica ad ogni deposito
var at{FABBRICHE,PRODOTTI,DEPOSITI} >= 0;
#Rappresenta il numero di camion necessari per trasportare tutto il prodotto lavorato di ogni fabbrica ai rispettivi magazzini
var nc{FABBRICHE,DEPOSITI} integer >= 0;
#Data una fabbrica, una macchina, un turno di lavoro e un tipo di acciaio, questa variabile logica binaria indica se é stata eseguita una lavorazione del prodotto indicato
#Si = 1         No = 0
var mAcciaio{FABBRICHE,MACCHINE,TURNI,PRODOTTI} binary;
#Data una fabbrica, un turno di lavoro e una macchina, questa variabile logica binaria indica se é stata eseguita una lavorazione con tale macchina
#Si = 1         No = 0
var mAttive{FABBRICHE,MACCHINE,TURNI} binary;
#Data una fabbrica, una macchina ed un tipo di acciaio, questa variabile logica binaria indica se la macchina indicata é stata impostata per la lavorazione del prodotto.
#Si = 1         No = 0
...
maximize profitto: 
			...
			#Costi delle pulizie delle macchine impostate alla produzione di un unico tipo di acciaio
...
#Questo vincolo fa si che per ogni fabbrica, macchina e turno, il tempo effettivo di lavorazione dell'acciaio sia sempre inferiore o uguale al tempo di lavoro dei dipendenti imposto dai sindacati
#In particolar modo tiene conto anche del tempo di pulizia della macchina solo se essa non é impostata.
...
#Questi due vincoli impongono alla variabile at un limite inferiore dovuto alla richiesta dei depositi e un vincolo superiore dovuto alla loro massima capienza
...
#Questo vincolo lega la variabile nc a quella at, permettendo così di definire per ogni fabbrica e deposito, il numero di camion necessari per trasportare tutto l'acciaio prodotto
#In particolare la variabile nc é un intero e poichè a parità di acciaio, l'aumentare del numero di camion diminuisce il profitto, in automatico il numero sarà l'intero superiore risultante dalla divisione della somma(prodotto)/capienza(camion)
...
#Questo vincolo fa si che data una fabbrica e una macchina, la variabile logica mImpostate possa risultare 1 solo su un unico prodotto.
